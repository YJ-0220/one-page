...
const toggleImageActive = async (id: string, isPopup: boolean, active: boolean): Promise<boolean> => {
  // 실제 API 호출 구현
...
const ContentManagement = () => {
  ...
  const handleToggleActive = async (id: string, isPopup: boolean, currentActive: boolean) => {
    try {
      ...
      if (result) {
        if (isPopup) {
          setPopupImages(popupImages.map(img => 
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center p-12">
        ...
        <span className="ml-3 text-gray-600">콘텐츠 데이터를 불러오는 중...</span>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 w-full">
      <div className="mb-6">
        <h3 className="text-xl font-semibold text-gray-800 mb-4">콘텐츠 관리</h3>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {popupImages.map((image) => (
                      <tr key={image.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                            onClick={() => handleToggleActive(image.id, true, image.active)}
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {slideImages.map((image) => (
                      <tr key={image.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                            onClick={() => handleToggleActive(image.id, false, image.active)}
                            className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-medium rounded-full ${
                              image.active
                                ? 'bg-green-100 text-green-800'
                                : 'bg-gray-100 text-gray-800'
