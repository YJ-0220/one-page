  ...
  const fetchContacts = async () => {
    ...
    try {
      ...
      if (err.message) {
        ...
        setError('문의 데이터를 불러오는 중 오류가 발생했습니다. 권한을 확인해주세요.');
      }
    } finally {
      setLoading(false);
    }
            ...
            <table className="w-full table-fixed divide-y divide-gray-200">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/3">메시지</th>
                  <th className="px-3 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-1/12">상태</th>
                  <th className="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">액션</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {contacts.map(contact => (
                  <tr key={contact._id} className={`hover:bg-gray-50 transition-colors ${!contact.isRead ? 'bg-blue-50' : ''}`}>
                    <td className="px-3 py-3 text-sm font-medium text-gray-900 truncate">{contact.name}</td>
                    <td className="px-3 py-3 text-sm text-gray-600 truncate">{contact.email}</td>
                    <td 
                    ...
                    >
                      <div className="line-clamp-2 group">
                        <span className="group-hover:underline">{contact.message}</span>
                        <span className="text-blue-500 ml-1 group-hover:underline text-xs font-medium">(자세히 보기)</span>
                      </div>
                    </td>
              ...
              <div className="mb-4">
                <div className="flex justify-between items-center">
                  <div className="text-sm text-gray-500">
                    작성일: {new Date(selectedContact.createdAt).toLocaleString()}
                  </div>
                  <div>
                    {selectedContact.isRead ? (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
